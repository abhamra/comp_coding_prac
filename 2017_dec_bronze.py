...
def main():
    ...
    #OTHER, EASIER WAY FOR ME

    # for i in range(2): #for each of the rectangles (first 2 billboards and then the truck)
    #     x1, y1, x2, y2 = map(int, input().split())
    #     # x1 += max_pos // 2
    #     # y1 += max_pos // 2
    #     # x2 += max_pos // 2
    ...
    #     for x in range(x1, x2):
    #         for y in range(y1, y2):
    #             visible[x][y] = True #this is a truth statement, so it checks if it is < 2, and when 3 !< 2, this is the case
    #             #of the truck, which is the last input. Because of this trick, we don't need to use an if statement
